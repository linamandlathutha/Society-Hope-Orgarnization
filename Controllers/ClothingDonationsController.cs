...
{
    public class ClothingDonationsController : Controller
    {

        private readonly ApplicationDbContext _context;
        private readonly UserManager<IdentityUser> _userManager;

        public ClothingDonationsController(ApplicationDbContext context, UserManager<IdentityUser> userManager)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index(string searchString)
        {
            try
            {
                ...
                IQueryable<ClothingDonation> reservationsQuery = _context.ClothingDonations;
                ...
                if (isAdmin)
                {
                    ...
                    {
                        reservationsQuery = reservationsQuery.Where(d => d.Email.Contains(searchString));
                    }
                }
                else
                {
                    // For non-admin users, filter by the logged-in user's email and the search string if provided
                    reservationsQuery = reservationsQuery.Where(d => d.Email == userEmail);
                    if (!string.IsNullOrEmpty(searchString))
                    {
                        reservationsQuery = reservationsQuery.Where(d => d.Email.Contains(searchString));
                    }
        ...
        public async Task<IActionResult> Create()
        {
            ...
            var groceryDonation = new ClothingDonation();
        ...
        public IActionResult CreateAdmin(ClothingDonation donation)
        {
            if (ModelState.IsValid)
            {
                _context.ClothingDonations.Add(donation);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(ClothingDonation clothingDonation)
        {
            if (ModelState.IsValid)
            {
               ...
               .FirstOrDefaultAsync(p => p.Email == User.Identity.Name);
                ...
                _context.ClothingDonations.Add(clothingDonation);
                await _context.SaveChangesAsync();

                // Send email confirmation
                await SendEmailConfirmation(clothingDonation);

                return RedirectToAction(nameof(Index));
            }

            return View(clothingDonation);
        }
        ...
        private async Task SendEmailConfirmation(ClothingDonation donation)
        {
            // Email configuration from appsettings.json
            var smtpServer = "smtp.gmail.com";
            var smtpPort = 587;
            var smtpUsername = "youremail@gmail.com";
            var smtpPassword = "emailpassword";
            ...
            var body = $"Dear {donation.FirstName},\n\n" +
                       $"Thank you for your generous donation of clothing items. " +
            ...
            public async Task<IActionResult> EditTrackingCode(int? id)
        {
            ...
            var groceryDonation = await _context.ClothingDonations.FindAsync(id);
        ...
        public async Task<IActionResult> EditTrackingCode(int id, UpdateTrackViewModel groceryDonation)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var donationToUpdate = await _context.ClothingDonations.FindAsync(id);
        ...
        private async Task SendEmailWithTrackingCode(ClothingDonation donation)
        {
            // Email configuration
            var smtpServer = "smtp.gmail.com";
            var smtpPort = 587;
            var smtpUsername = "youremail@gmail.com";
            var smtpPassword = "youremailpassword"; // Secure your password
            var fromAddress = "youremail@gmail.com";
        ...
        {
            return _context.ClothingDonations.Any(e => e.Id == id);
        }
